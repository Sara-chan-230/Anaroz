...
import { getCampaignbyId } from "../../Redux/APICalls/campaignsCall";

const CampaignPage = () => {
  const [amount, setAmount] = useState(null);
  const [open, setOpen] = useState(false);
  const { isDonate, isVolunteer, setLoadign } = useSelector(
    (state) => state.help
  );
  const { user } = useSelector((state) => state.auth);
  const { camp } = useSelector((state) => state.campaigns);
  ...
  const handleSubmit = (e) => {
    ...
    if (user._id === null) return toast.error("you should login firest");
    dispatch(
      Donate({
        numberOfHopePieces: amount,
        userId: user._id,
        campaignId: idCamp,
      })
    );
  ...
  const handleVolunteer = () => {
    dispatch(Volunteer({ userId: user._id, campaignId: idCamp }));
  };

  useEffect(() => {
    window.scrollTo(0, 0);

    console.log("Campaign ID:", idCamp);
    if (idCamp) {
      dispatch(getCampaignbyId(idCamp));
    }
  }, [idCamp, dispatch]);
                ...
                <div className="relative w-28 h-28 shrink-0">
                  <svg className="w-full h-full" viewBox="0 0 36 36">
                    ...
                    <circle
                      ...
                      strokeDashoffset={
                        100 - Number((camp.wallet / camp.budget) * 100)
                      }
                      strokeLinecap="round"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    {Math.floor((camp.wallet / camp.budget) * 100)}
                    </span>
              ...
              <div className="md:w-[30%] flex flex-col sticky top-6 h-fit">
                <div className="bg-white rounded-lg shadow-xl p-6 space-y-6">
                  ...
                  <section className="p-5 bg-amber-50 rounded-lg border border-amber-100 ">
                    ...
                    <p className="text-sm text-gray-700 mb-5">
                      Your time and skills can rebuild hope. Choose how you'd
                      like to contribute:
                    </p>
